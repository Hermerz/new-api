...
func (w *wsResponseWriter) Write(data []byte) (int, error) {
	// Prevent unbounded buffer growth from a stuck/misbehaving upstream
	if w.buf.Len() > maxBufSize {
		logger.LogError(nil, "ws response buffer overflow, clearing")
		w.buf.Reset()
	}
...
func RelayResponsesWS(c *gin.Context) {
	...
	// consuming and billing tokens for a stream nobody receives.
	...
	// Read the JSON request from the first WebSocket message
